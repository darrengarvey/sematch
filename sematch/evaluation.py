...
def generate_report(gold, predict, labels, detailed=True):
    ...
    if detailed:
        for avg in ['macro', 'micro', 'weighted']:
            prec, recall, fscore, support = precision_recall_fscore_support(gold, predict, average=avg)
            print('{} averages: precision={}, recall={}, fscore={}, support={}'.format(avg, prec, recall, fscore, support))
    else:
        import warnings
...
class WordSimDataset:
    ...
    def save_result(self, cor, sim_values, sim_name, dataset_name):
        ...
        """
        data = ["%.3f" % cor]
        data += list(map(lambda x: "%.3f" % x, sim_values))
